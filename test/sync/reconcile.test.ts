...
import { SyncNode } from '../../src/sync/xsync';
...
describe('applyStateTree create', () => {
    ...
    it('creates client units under the reconcile root with state applied', () => {
        ...
        expect(Unit.syncData(child).id).toBe(1);
        expect(Unit.syncData(child).state).toEqual({ value: 7 });
    });
    ...
    it('creates nested replica units honoring parent', () => {
        ...
        expect(Unit.syncData(view._.children[0]).id).toBe(1);
        expect(Unit.syncData(view._.children[0]._.children[0]).id).toBe(2);
    });
});
...
describe('applyStateTree update', () => {
    ...
    it('updates existing unit in place without recreating it', () => {
        ...
        expect(Unit.syncData(first).state).toEqual({ value: 2 });
        expect(view._.children.length).toBe(1);
    });
    ...
    it('drops keys the authoritative state no longer carries, keeping the state object identity', () => {
        ...
        const state = Unit.syncData(view._.children[0]).state;
        socket.fire('sync', [{ id: 1, name: 'Box', parent: null, state: { value: 2 } }]);
        expect(state).toEqual({ value: 2 });          // 'flag' が消える
        expect('flag' in state).toBe(false);
        expect(Unit.syncData(view._.children[0]).state).toBe(state);   // 参照は据え置き（本体が掴んだクロージャが生きる）
    });
});
...
describe('applyStateTree remove', () => {
    ...
    it('finalizes replica units whose id disappears from the tree', () => {
        ...
        const removed = view._.children.find(c => Unit.syncData(c).id === 2)!;
