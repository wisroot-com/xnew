...
//----------------------------------------------------------------------------------------------------
// ルームステータス（xsync.session.room / clients / myself / sync.statusupdate）
//   - server: メンバ接続/切断で台帳(session.clients)が更新され、サブツリーへ sync.statusupdate が配られる。
//   - client: server からの status 配信を取り込み、sync.statusupdate を配る。session.myself は自分自身の ClientStatus。
//----------------------------------------------------------------------------------------------------

describe('room status (session.clients / session.myself / sync.statusupdate)', () => {
    ...
    it('server status.clients tracks members; sync.statusupdate fires on connect/disconnect', () => {
        ...
        bootServer({ io: hub.io }, function Server(unit: Unit) {
            xsync.server(() => { unit.on('sync.statusupdate', () => snapshots.push(xsync.session.clients.map((c) => c.id))); });
        });
    ...
    it('client reads myself / clients via broadcast', () => {
        ...
        bootClient({ socket: hub.connect('c1') }, function Client(unit: Unit) {
            xsync.client(() => { unit.on('sync.statusupdate', () => { myself = xsync.session.myself; clients = xsync.session.clients; }); });
        });
