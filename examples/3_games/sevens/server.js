//----------------------------------------------------------------------------------------------------
// sevens（server 側）— express で静的配信し、socket.io で 1 つの固定ルームを同期する。
//   ロビーは無く、全接続が固定ルーム 'main' に入る。server は Game を 1 度 boot するだけ。Node 実行なので
//   Game の xsync.server 分岐（配札 + 手番管理 + ルール判定）だけが動く。ゲーム本体 game.js は無改変。
//----------------------------------------------------------------------------------------------------
...
const room = { id: 'main', name: '七並べルーム', count: 0 };

// server root: 固定ルームの Game を 1 度だけ boot する（inline は unit を暗黙 return しないようブロック本体で）。
xnew(() => { xsync.boot({ io, room }, Game); });

httpServer.listen(PORT, () => {
    console.log(`[sevens] socket.io server on http://localhost:${PORT}/`);
    console.log(`[sevens] http://localhost:${PORT}/index-multiframe.html を開くと 4 画面（4 プレイヤー）を並べて確認できます`);
});
