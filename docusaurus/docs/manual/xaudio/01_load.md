...
`xaudio.load` は音声ファイルを取得・デコードして再生するトラックを作成します。`play` / `pause` で駆動し、パッケージ共有のオーディオバスへミックスされます。トラックの解放は現在のスコープに作られる unit に紐づくため、呼び出し元の unit（シーンなど）が破棄されると、確保していた Web Audio ノードも自動的に解放されます。
