...
// is a rounded background with a knob sliding between the edges, so callers get the familiar
...
//                 decorate the container; designs: { background?, knob? } — a Design
...
export function InputSwitch(unit: xnew.Unit,
    ...
    { value?: boolean, className?: string, style?: string, designs?: { background?: Design, knob?: Design }, [key: string]: any } = {}
) {
    const cls = xnew.css({
        ...
        // aspect-ratio keeps the knob square at any background size, so the slide needs no size math
        knob: {
            layer: 'xbasics',
            body: `
        ...
        // invisible native control stretched over the background
        input: {
            layer: 'xbasics',
            body: `
    ...
    const background = xnew.nest({ tag: 'div', className: `${cls.background} ${designs.background?.className ?? ''}`, style: designs.background?.style });
