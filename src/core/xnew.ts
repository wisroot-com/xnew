...
export const xnew = Object.assign(
    ...
    }) as unknown as XnewBase,
    {
        ...
        // Registers pseudo-scoped CSS: each key is a local name, always renamed to a page-unique one (scoping is mandatory — invalid keys throw). A value is either a declaration block string, wrapped as .xnewN-key { … } (native nesting works inside, e.g. &:hover / @media), or an object { layer?, type?, body }: type names an at-rule without '@' to hang the generated name on ({ type: 'keyframes', body } → @keyframes xnewN-key; absent: a class rule), and layer wraps that entry in @layer (xbasics entries set layer: 'xbasics'). $key inside a body references another entry's generated name (unknown references throw). Returns { key: generatedName } to embed in tag strings; the injected <style> is shared per definition and removed when the last unit using it finalizes.
